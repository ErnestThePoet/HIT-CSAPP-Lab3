...
void BlurImpls::BlurAVXOpenMP(const Bitmap& bitmap, Bitmap& dest_bitmap)
{
	// pixels per inner loop
	constexpr int32_t kWpl = 32;
	...
	uint8_t* const pixel_data = bitmap.pixel_data();
	uint8_t* dest_pixel_data = dest_bitmap.pixel_data();
...
#pragma omp parallel for
	for (int32_t i = 1; i < height_px - 1; i++)
	{
		...
		for (int32_t j = bytes_per_px; j < inner_loop_ubound; j += kWpl)
		{
			...
			__m256i packed_pixel_data_upper =
				_mm256_loadu_si256((__m256i*)(pixel_data + upper_row_offset + j));
			...
			_mm256_storeu_si256(
				(__m256i*)(dest_pixel_data + current_total_offset), packed_avg_pixel_data);
		}
	}
}
#endif
...
void BlurImpls::BlurNeon(Bitmap& bitmap)
{
	...
	for (int32_t i = 1; i < height_px - 1; i++)
	{
		...
		for (int32_t j = bytes_per_px; j < inner_loop_ubound; j += kWpl)
		{
			...
			uint8x16_t packed_pixel_data_upper =
				vshrq_n_u8(vld1q_u8(upper_row_pixels + j), 2);

			uint8x16_t packed_pixel_data_l =
				vshrq_n_u8(vld1q_u8(current_row_pixels + j - bytes_per_px),2);

			uint8x16_t packed_pixel_data_r =
				vshrq_n_u8(vld1q_u8(pixel_data + current_total_offset + bytes_per_px), 2);

			uint8x16_t packed_pixel_data_lower =
				vshrq_n_u8(vld1q_u8(pixel_data + lower_row_offset + j), 2);


			uint8x16_t packed_avg_pixel_data = vaddq_u8(
				vaddq_u8(
					packed_pixel_data_upper,
					packed_pixel_data_l
				),
			...
			vst1q_u8(pixel_data + current_total_offset, packed_avg_pixel_data);
		}
...
void BlurImpls::BlurNeonOpenMP(const Bitmap& bitmap, Bitmap& dest_bitmap)
{
	// pixels per inner loop
	constexpr int32_t kWpl = 16;
...
#pragma omp parallel for
	for (int32_t i = 1; i < height_px - 1; i++)
	{
		...
		for (int32_t j = bytes_per_px; j < inner_loop_ubound; j += kWpl)
		{
			...
			uint8x16_t packed_pixel_data_upper =
				vshrq_n_u8(vld1q_u8(pixel_data + upper_row_offset + j), 2);
			...
			vst1q_u8(dest_pixel_data + current_total_offset, packed_avg_pixel_data);
		}
	}
}
#endif
